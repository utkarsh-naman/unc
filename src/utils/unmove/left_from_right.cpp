...
State_str plr(const State_tuple& state)
{
    State_str lr_prev_state_str = {0};
    Player attacker;
    Player victim;
    ...
    if (get<0>(victim) - get<1>(attacker) == 0) return lr_prev_state_str;
    // if previous attacking hand is 0 then how could he hit
    if (get<1>(attacker) == 0) return lr_prev_state_str;
    get<0>(victim) = mod( static_cast<short>(get<0>(victim) - get<1>(attacker)), MAX_FINGER);
